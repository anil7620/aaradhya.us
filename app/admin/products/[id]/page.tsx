...
import { CheckCircle2, Loader2, Tag, Trash2, Upload, X, GripVertical, Edit2, ArrowUp, ArrowDown, Link as LinkIcon, Star } from 'lucide-react'
...
function ImagePreview({ 
  ...
  const handleSaveUrl = () => {
    if (onEditUrl && editedUrl.trim() && editedUrl !== image) {
      ...
      setImageError(false) // Reset error state when URL is updated
    }
    setIsEditing(false)
  }
...
export default function AdminEditProductPage() {
    ...
    const fetchProduct = async () => {
      ...
      try {
        ...
        setForm({
          ...
          mrp: data.product.mrp ? String(data.product.mrp) : '',
          category: data.product.category,
          stock: String(data.product.stock),
          images: (data.product.images || []).filter((img: string) => img && img.trim()),
          colors: data.product.colors || [],
          fragrances: data.product.fragrances || [],
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Retry logic for failed PUT requests
    const attemptUpdate = async (retryCount = 0): Promise<void> => {
      try {
        ...
        if (!res.ok) {
          // If product not found and we haven't retried, wait and retry once
          if (res.status === 404 && retryCount === 0) {
            console.log('Product not found on first attempt, retrying after 500ms...')
            await new Promise(resolve => setTimeout(resolve, 500))
            return attemptUpdate(1)
          }
          
          // If product not found after retry, redirect to products list
          if (res.status === 404) {
            setMessage({ 
              type: 'error', 
              text: 'Product not found. It may have been deleted. Redirecting to products list...' 
            })
